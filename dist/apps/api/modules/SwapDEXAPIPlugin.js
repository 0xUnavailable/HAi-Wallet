...
exports.SwapDEXAPIPlugin = void 0;
...
async function getIndicativePrice(params) {
    ...
    const priceResponse = await (0, node_fetch_1.default)(`${ZEROX_PRICE_API_BASE}?${priceParams.toString()}`, { headers });
...
async function setTokenAllowance(params) {
    ...
    const walletClient = (0, viem_1.createWalletClient)({
        account: (0, accounts_2.privateKeyToAccount)(privateKey),
        chain: {
            id: parseInt(chainId),
            name: chainId === '1' ? 'Ethereum' : chainId === '10' ? 'Optimism' : 'Arbitrum',
            nativeCurrency: { name: 'Ether', symbol: 'ETH', decimals: 18 },
    ...
    // Set up contracts
    const Permit2 = (0, viem_1.getContract)({
        address: PERMIT2_ADDRESS,
        abi: exchangeProxyAbi,
        client: publicClient,
    });
    const sellTokenContract = (0, viem_1.getContract)({
        address: sellToken,
        abi: erc20Abi,
        client: publicClient,
    });
    // Check allowance
    const currentAllowance = await sellTokenContract.read.allowance([account.address, PERMIT2_ADDRESS]);
    if (BigInt(currentAllowance.toString()) < BigInt(sellAmount)) {
        try {
            // Approve Permit2 to spend the token
            const { request } = await sellTokenContract.simulate.approve([PERMIT2_ADDRESS, viem_1.maxUint256]);
...
async function getFirmQuote(params) {
    ...
    const response = await (0, node_fetch_1.default)(`${ZEROX_QUOTE_API_BASE}?${quoteParams.toString()}`, { headers });
...
async function signAndExecuteTransaction(params) {
    ...
    if (quote.permit2?.eip712) {
        ...
        signature = await (0, accounts_1.signTypedData)({
    ...
    if (quote.permit2?.eip712) {
        ...
        transactionData = (0, viem_1.concat)([transactionData, signatureLengthInHex, signature]);
    }
    // Execute transaction
