...
def extract_intent_parameters(doc):
    ...
    amount_pattern = [{"IS_DIGIT": True}, {"TEXT": {"IN": ["ETH", "USDC", "USDT"]}}]
    token_pattern = [{"TEXT": {"IN": ["ETH", "USDC", "USDT"]}}]
    recipient_pattern = [
        {"LOWER": {"IN": ["to", "for"]}},
        {"IS_ALPHA": True, "TEXT": {"NOT_IN": ["Ethereum", "Base", "Arbitrum", "on", "from"]}}
    ]
    address_pattern = [
        {"LOWER": {"IN": ["to", "for"]}},
    ...
    source_network_pattern = [
        {"LOWER": "from"},
        {"TEXT": {"IN": ["Ethereum", "Base", "Arbitrum"]}},
    ...
    for i, intent_pos in enumerate(intent_positions):
        ...
        for match_id, start, end in scope_matches:
            ...
            if match_label == "AMOUNT":
                amount = span[0].text
                if end < len(intent_scope) and intent_scope[end-1].text in ["ETH", "USDC", "USDT"]:
                ...
                network_keywords = ["Ethereum", "Base", "Arbitrum"]
        ...
        # Apply network logic based on intent type
        if intent in ["Transfer", "Swap", "Query"]:
        ...
        # Only add intent if it has some meaningful parameters (tokens or recipient)
        if parameters["tokens"] or parameters["to"] or parameters["query_type"]:
